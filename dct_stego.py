...
def main():
    inFile = 'img/lenna.png'
    message = 'Encryption requires an input string'
    outFile = None
    msgFile = None

    encrypt = True

    root_folder = 'images/stego/'
    start_time = time.clock()
    print("Starting dct for ", root_folder)
    sub_folders = [os.path.join(root_folder, f) for f in os.listdir(root_folder) if
                   os.path.isdir(os.path.join(root_folder, f))]
    for sub_folder in sub_folders:
        for im_path in glob.glob(sub_folder + "/*.tiff"):
            x = DCT(im_path)
            secret = x.DCTEn(message, im_path)
